...
function convertToRomanNumerals(num) {
  ...
  return romanNum + romanNumerals[arabicNum];
}
...
function getBalanceIndex(arr) {
  ...
  for (let middle = 1; middle < arr.length; middle += 1) {
    let leftSum = 0;
    let rightSum = 0;
...
function getSpiralMatrix(size) {
  const matrixElementsCount = size * size;
  const spiralMatrix = [];
  ...
  while (matrixElem <= matrixElementsCount) {
    for (let i = x1; i <= x2; i += 1) {
      spiralMatrix[y1][i] = matrixElem;
      matrixElem += 1;
    }
...
function getNearestBigger(number) {
  ...
  const left = [];
  for (let i = 0; i < numIndex; i += 1) {
    left.push(numberArr[i]);
  }

  const right = numberArr.splice(left.length).sort((a, b) => a - b);
  ...
  const right2 = [...right.splice(0, numIndex2), ...right.splice(1)];
  return Number([...left, num2, ...right2].join(''));
}
